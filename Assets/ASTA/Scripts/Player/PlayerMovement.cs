...
public class PlayerMovement : MonoBehaviour {
	...
	void FixedUpdate() {
		...
		if(animator) {
			...
			animator.SetAnimatorBool("Falling", !isGrounded && rb.linearVelocity.y < 0.1f && playerState.currentState != UNITSTATE.KNOCKDOWN);
	...
	void MoveGrounded() {
		...
		if(rb != null && (inputDirection.sqrMagnitude > 0) && !WallInFront() && PlayerInsideCamViewArea()) {
			SetVelocity(new Vector3(inputDirection.x * -walkSpeed, rb.linearVelocity.y + Physics.gravity.y * Time.fixedDeltaTime, inputDirection.y * -ZSpeed));
			setPlayerState(UNITSTATE.WALK);
		} else {
			SetVelocity(new Vector3(0, rb.linearVelocity.y + Physics.gravity.y * Time.fixedDeltaTime, 0));
			setPlayerState(UNITSTATE.IDLE);
		}
		...
		if(!PlayerInsideCamViewArea() && Mathf.Abs(inputDirection.y) > 0) {
			...
			SetVelocity(new Vector3(dirToCam.x, rb.linearVelocity.y + Physics.gravity.y * Time.fixedDeltaTime, dirToCam.z));
		}
		...
		if(animator) animator.SetAnimatorFloat("MovementSpeed", rb.linearVelocity.magnitude);
	...
	void MoveAirborne(){
		if(!WallInFront() && PlayerInsideCamViewArea()) {
			...
			float xpeed = Mathf.Clamp(rb.linearVelocity.x + AirMaxSpeed * dir * Time.fixedDeltaTime * AirAcceleration, -AirMaxSpeed, AirMaxSpeed);

			//apply movement
			if(!updateVelocity){
				if(AllowDepthJumping) {
					SetVelocity(new Vector3(xpeed, rb.linearVelocity.y + Physics.gravity.y * Time.fixedDeltaTime, -inputDirection.y * ZSpeed));
				} else {
					SetVelocity(new Vector3(xpeed, rb.linearVelocity.y + Physics.gravity.y * Time.fixedDeltaTime, 0));
				}
			}
		} else {
			if(!updateVelocity) SetVelocity(new Vector3(0f, rb.linearVelocity.y + Physics.gravity.y * Time.fixedDeltaTime, 0f));
		}
	}
